...
void TestCppClient::whatIfSamples()
{
    ...
    //! [whatiforder]
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(), OrderSamples::WhatIfLimitOrder("BUY", DecimalFunctions::stringToDecimal("200"), 120));
    //! [whatiforder]
...
void TestCppClient::openOrder( OrderId orderId, const Contract& contract, const Order& order, const OrderState& orderState) {
    printf( "OpenOrder. PermId: %s, ClientId: %s, OrderId: %s, Account: %s, Symbol: %s, SecType: %s, Exchange: %s:, Action: %s, OrderType:%s, TotalQty: %s, CashQty: %s, "
        ...
        "FAGroup: %s, FAMethod: %s, CustomerAccount: %s, ProfessionalCustomer: %s\n",
        ...
        order.faGroup.c_str(), order.faMethod.c_str(), order.customerAccount.c_str(), (order.professionalCustomer ? "true" : "false"));
}
//! [openorder]
